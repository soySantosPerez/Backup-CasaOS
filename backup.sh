#!/usr/bin/env bash
# casaos-backup — full CasaOS backup with transactional consistency
#
...
# Restart stopped containers and clean up on any exit
cleanup() {
  local exit_code=$?
  if [[ ${#STOPPED_CONTAINERS[@]} -gt 0 ]]; then
    log "Restarting stopped containers..."
    for c in "${STOPPED_CONTAINERS[@]}"; do
      docker start "$c" >/dev/null 2>&1 || log "WARN: could not restart $c"
    done
    STOPPED_CONTAINERS=()
  fi
  for f in "${PG_DUMPS_TAKEN[@]:-}"; do
    [[ -f "$f" ]] && rm -f "$f"
  done
  rm -rf "$WORK_DIR"
  exit "$exit_code"
...
# Is the NAS / destination accessible?
...
if ! docker ps >/dev/null 2>&1; then
  fail "Cannot talk to Docker. Is the Docker daemon running?"
fi

############################################
# Phase 1: pg_dump for each declared DB
############################################

if [[ -n "$PG_DUMP_DBS" ]]; then
  log "--- Phase 1: pg_dump ---"
  IFS=',' read -ra DBS <<< "$PG_DUMP_DBS"
  for entry in "${DBS[@]}"; do
    entry=$(echo "$entry" | xargs)  # trim
    IFS=':' read -r CONT USER DBNAME <<< "$entry"
    log "Dumping $DBNAME from container $CONT (user $USER)..."

    DUMP_FILENAME="_predump_${DBNAME}_${TIMESTAMP}.dump"
    if docker exec "$CONT" pg_dump -U "$USER" -d "$DBNAME" -F c \
         -f "/var/lib/postgresql/data/${DUMP_FILENAME}" 2>&1; then
      MOUNT_SRC=$(docker inspect "$CONT" --format \
        '{{range .Mounts}}{{if eq .Destination "/var/lib/postgresql/data"}}{{.Source}}{{end}}{{end}}' 2>/dev/null)
      if [[ -n "$MOUNT_SRC" ]]; then
        FULL_PATH="${MOUNT_SRC}/${DUMP_FILENAME}"
        log "  OK -> $FULL_PATH"
        PG_DUMPS_TAKEN+=("$FULL_PATH")
      else
        log "WARN: dump done but couldn't locate file on host"
      fi
    else
      fail "pg_dump failed for $DBNAME"
    fi
  done
else
  log "--- Phase 1: pg_dump (skipped, PG_DUMP_DBS empty) ---"
fi
...
log "--- Phase 2: stopping containers ---"
...
############################################
# Phase 5: transfer to destination
############################################

log "--- Phase 5: moving backup to destination ---"
...
############################################
# Phase 6: rotation (keep N most recent)
############################################

log "--- Phase 6: rotation ---"
...
############################################
# Phase 7: clean temporary pg_dumps
############################################

log "--- Phase 7: cleanup temporary dumps ---"
for f in "${PG_DUMPS_TAKEN[@]:-}"; do
  if [[ -f "$f" ]]; then
    rm -f "$f"
    log "Removed $f"
